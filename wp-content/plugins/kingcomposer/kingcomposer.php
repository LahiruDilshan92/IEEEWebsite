<?php
/*
Plugin Name: King Composer
Plugin URI: http://KingComposer.com/
Description: King Composer is the most professional WordPress page builder plugin, it's lightweight and high efficiency to help you build any layout design quickly.
Version: 2.2.0.5
Author: King-Theme
...
class KingComposer{
	...
	*/
	...
	public function init(){
		...
		add_action( 'kc_before_footer', array( &$this, 'convert_maps' ) );
		add_action( 'kc_after_footer', array( &$this, 'convert_paramTypes' ) );
		...
		foreach( $map as $base => $atts )
		{
			if( is_array( $atts ) ){
				...
				if( isset( $atts['views'] ) && !empty( $atts['views']['sections'] ) )
					array_push( $this->maps_views, $atts['views']['sections'] );
			}
		}
	}
	...
	public function convert_paramTypes(){
		...
		*/
		...
		echo '<script type="text/html" id="tmpl-kc-field-type-undefined-template"><input name="{{data.name}}" class="kc-param" value="{{data.value}}" type="text" /></script>';
		
		echo '<script type="text/javascript">var kc_param_types_support = '.json_encode( $type_support ).'</script>';
	...
	public function get_default_atts( $params ){
		...
		if( isset( $this->maps[$sc] ) ){
			...
			return shortcode_atts( $pairs, $reparams, $sc );
...
class kc_load_shortcodes{
	...
	public function __call( $func, $params ){
        ...
        if( !file_exists( $path ) )
        	return 'KingComposer Error: could not find shortcode template: '.$path;
