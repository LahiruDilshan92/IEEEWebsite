...
( function($){
	...
	$().extend( kc.tools, {
		...
		popup : new kc.backbone.views( 'no-model' ).extend({
			...
			render : function( el, atts ){
				...
				if( atts.scrollTo === true ){
					setTimeout( function( el, atts ){
						...
						if( wh > 800 )
							el.find('.m-p-body').css({ 'max-height': (wh - 250)+'px' });
							
						var newrect = el.get(0).getBoundingClientRect();
							
						if( ( atts.top > wheight && el.height()+atts.top > wsct + wh && atts.bottom == 0 ) )
							kc.ui.scrollAssistive( (atts.top - 50), false );
						else if( newrect.top < 0 )
							kc.ui.scrollAssistive( (wsct+newrect.top) - 50, false );
			...
			cancel : function( e ){
				...
				if( $('#instantSaving').length > 0 || kc.tools.popup.no_close === true ){
				...
				var el = $(this).closest('.kc-params-popup'), keepCurrent = el.data('keepCurrentPopups'),
	...
	kc.tools.base64.stringBuffer.prototype.append = function append(string) {
	    this.buffer.push(string);
	    return this
	};
	
	kc.tools.base64.stringBuffer.prototype.toString = function toString() {
	    return this.buffer.join("")
	};
	
	kc.tools.base64.utf8EncodeEnumerator.prototype = {
	...
	kc.tools.base64.decodeEnumerator.prototype = {
	    ...
	    moveNext: function() {
	        if (this._buffer.length > 0) {
	            ...
	            var enc1 = kc.tools.base64.codex.indexOf(this._input.charAt(++this._index));
	            var enc2 = kc.tools.base64.codex.indexOf(this._input.charAt(++this._index));
	            var enc3 = kc.tools.base64.codex.indexOf(this._input.charAt(++this._index));
	            var enc4 = kc.tools.base64.codex.indexOf(this._input.charAt(++this._index));
