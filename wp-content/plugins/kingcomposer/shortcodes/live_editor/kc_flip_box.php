...
var atts = ( data.atts !== undefined ) ? data.atts : {},
	...
	back_data = '',
	link = '#',
	image_data = '<?php echo KC_URL.'/assets/images/get_start.jpg'; ?>',
	element_atttribute = [],
	el_classess = [ 'kc-flipbox', 'kc-flip-container', custom_class ];
...
if( atts['image'] !== undefined && atts['image'] !== '' )
	image_data = '<?php echo admin_url('/admin-ajax.php?action=kc_get_thumbn&id='); ?>'+atts['image']+'&size='+image_size;

if( atts['title'] !== undefined && atts['title'] !== '' )
	back_data += '<h3>'+atts['title']+'</h3>';

if( atts['description'] !== undefined && atts['description'] !== '' )
	back_data += '<p>'+kc.tools.base64.decode( atts['description'] )+'</p>';

if( atts['show_button'] !== undefined && atts['show_button'] == 'yes' ){
	
	text_on_button = '<?php _e('Read more', 'kingcomposer'); ?>';
...
<div {{{element_atttribute.join(' ')}}}>
	...
	<style type="text/css">
	<#
		var bg_backside = kc.std( atts, 'bg_backside', '#86c724' ),
			text_color = kc.std( atts, 'text_color', '#FFFFFF'),
			button_bg_color = kc.std( atts, 'button_bg_color', 'transparent'),
			text_button_color = kc.std( atts, 'text_button_color', '#FFFFFF'),
			button_bg_hover_color = kc.std( atts, 'button_bg_hover_color', '#FFFFFF'),
			text_button_color_hover = kc.std( atts, 'text_button_color_hover', '#86c724');
	#>
