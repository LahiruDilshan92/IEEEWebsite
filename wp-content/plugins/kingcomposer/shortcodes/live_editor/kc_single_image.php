...
var output = '', atts = ( data.atts !== undefined ) ? data.atts : {},
	default_src = '<?php echo KC_URL.'/assest/images/get_start.jpg'; ?>',
	image_source = ( atts['image_source'] !== undefined ) ? atts['image_source'] : '',
	element_attributes = [],
	image_attributes = [],
	css_classes = [ 'kc_shortcode', 'kc_single_image' ],
	image_url = '';
	caption = '',
...
else
{
	...
	if( image_source == 'media_library' )
	{
		...
		image_full = '<?php echo admin_url('/admin-ajax.php?action=kc_get_thumbn&size=full&id='); ?>'+image_id;
	}
	else
	{
		<?php
	...
	image_full = '<?php echo admin_url('/admin-ajax.php?action=kc_get_thumbn&size=full&id='); ?>'+image_id;
	image_attributes.push( 'src="<?php echo admin_url('/admin-ajax.php?action=kc_get_thumbn&id='); ?>'+image_id+'&size='+image_size+'"' );
