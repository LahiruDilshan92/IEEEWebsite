...
$output = $title = $wrap_class = $contact_form = $disable_wheel_mouse = $custom_class = '';
$map_width = '100%';
$map_height = '250px';
...
if( !empty($show_ocf) && 'yes' === $show_ocf ){
    if(!empty( $contact_form_sc )){
        ...
        $contact_form .= do_shortcode( base64_decode( $contact_form_sc ) );
        $contact_form .= '</div>';
...
$map_location = base64_decode( $map_location );
